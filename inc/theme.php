...
add_action( 'wp_enqueue_scripts'     , 'epic_base_load_fonts' );
...
function epic_base_default_menu() {
	$html = '<div id="primary-menu" class="collapse navbar-collapse"> <ul id="menu-all-pages" class="nav navbar-nav"> <li class="menu-item menu-item-type-post_type menu-item-object-page"><a title="' . esc_attr__( 'Home', 'epic-base' ) . '" href="' . esc_url( home_url() ) . '">' . esc_attr__( 'Home', 'epic-base' ) . '</a></li> <li class="menu-item menu-item-type-post_type menu-item-object-page"><a title="' . esc_attr__( 'About', 'epic-base' ) . '" href="#">' . esc_attr__( 'About', 'epic-base' ) . '</a></li> <li class="menu-item menu-item-type-post_type menu-item-object-page"><a title="M' . esc_attr__( 'Blog', 'epic-base' ) . '" href="#">' . esc_attr__( 'Blog', 'epic-base' ) . '</a></li> </ul> </div>';
...
function epic_base_register_layouts() {
	...
	hybrid_register_layout(
		'1c',   array(
			'label' => esc_html__( '1 Column',                     'epic-base' ),
			'image' => '%s/images/layouts/1c.png',
		)
	);
	hybrid_register_layout(
		'2c-l', array(
			'label' => esc_html__( '2 Columns: Content / Sidebar', 'epic-base' ),
			'image' => '%s/images/layouts/2c-l.png',
		)
	);
	hybrid_register_layout(
		'2c-r', array(
			'label' => esc_html__( '2 Columns: Sidebar / Content', 'epic-base' ),
	...
	if ( get_post_gallery() ) :
		...
		foreach ( $gallery['src'] as $src ) {
			...
			<a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php the_title_attribute(); ?>">
				<img src="<?php echo esc_url( $src ); ?>" class="caviar_img" alt="<?php the_title_attribute(); ?>" width="<?php echo esc_attr( $width ); ?>" height="<?php echo esc_attr( $height ); ?>"/>
			</a>
			</figure>
	...
	if ( has_post_thumbnail() ) {
		...
		foreach ( $sizes as $size ) {
			...
			if ( ! empty( $image ) && ( true === $image[3] || 'full' == $size ) ) {
				...
				$label = sprintf( esc_html__( '%1$s &#215; %2$s', 'epic-base' ), number_format_i18n( $image[1]  ), number_format_i18n( $image[2]  ) );
